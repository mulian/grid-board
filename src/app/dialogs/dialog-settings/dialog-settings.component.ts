...
import { selectDialogAll, DialogModelSettings, selectDialogSettings, SelectSettingsTab } from "../../stores/dialog"
import { dialogInitialStateSettings } from "../../stores/dialog/dialog.initial.state"
import { SettingsTab } from "../../stores/dialog/dialog.model"
...
export class DialogSettingsComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.settingsSubscription = this.store
            .pipe(select(selectDialogSettings))
            .subscribe((dialogSettings: DialogModelSettings) => {
                if (dialogSettings.currentTab != this.selected.value) {
                    //only if tab is not already selected
        ...
        this.selected.valueChanges.subscribe((value: number) => {
            if (this.dialogSettings.currentTab != value) {
                //only if tab is not already selected
